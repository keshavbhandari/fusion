...
def plot_muspy_comparison(data1, data2, title: Optional[str] = None, save_img_path: Optional[str] = None):
    ...
    # Plotting bars
    plt.bar(x - width / 2, values1, width, label=data1['info']["song_tag"].split('_')[0], color='#FF6F61')
    plt.bar(x + width / 2, values2, width, label=data2['info']["song_tag"].split('_')[0], color='#42A5F5')
