...
pitch_index_to_sharp_names = np.array(['C', 'C#', 'D', 'D#', 'E', 'F', 'F#', 'G', 'G#', 'A', 'A#', 'B'])

pitch_index_to_flat_names = np.array(['C', 'D-', 'D', 'E-', 'E', 'F', 'G-', 'G', 'A-', 'A', 'B-', 'B'])
...
def cal_diameter(piano_roll: PianoRoll, key_index: int, key_change_beat=-1, changed_key_index=-1) -> List[int]:
...
def cal_key(piano_roll: PianoRoll, key_names: str, end_ratio=0.5) -> Tuple[str, int, int]:
...
def cal_tension(file_name: str,
    ...
    # pickle.dump(total_tension, open(os.path.join(new_output_folder, name_split[0] + '.tensile'), 'wb'))

    # pickle.dump(diameters, open(os.path.join(new_output_folder, name_split[0] + '.diameter'), 'wb'))
    # pickle.dump(centroid_diff, open(os.path.join(new_output_folder, name_split[0] + '.centroid_diff'), 'wb'))

    # pickle.dump(window_time[:len(total_tension)],
    #             open(os.path.join(new_output_folder, name_split[0] + '.time'), 'wb'))
    ...
    return [
        total_tension, diameters, centroid_diff, key_name, change_time, key_change_bar, changed_key_name,
        new_output_folder, window_time
    ]
...
def get_key_index_change(pm: PianoRoll, start_time: float, sixteenth_time: ndarray):
...
def cal_centroid(piano_roll: PianoRoll, key_index: int, key_change_beat=-1, changed_key_index=-1):
    centroids = []
...
def detect_key_change(key_diff: ndarray, diameter: ndarray, start_ratio=0.5) -> int:
    # 8 bar window
    key_diff_ratios = []
    diameter_ratios = []
    fill_one = False
...
def get_beat_time(pm: PrettyMIDI, beat_division=4) -> Tuple[ndarray, ndarray, ndarray, List[int], List[int]]:
    beats = pm.get_beats()
...
def extract_notes(file_name: str,
                  track_num: int) -> Tuple[PrettyMIDI, PianoRoll, ndarray, ndarray, ndarray, List[int], List[int]]:
    try:
...
def calculate_tonal_tension(file_name: str, output_folder: str, key_name: str, key_changed: bool, win_sz: int = -1):
    ...
    if len(window_time) != len(diameters):
        window_time = window_time[:len(diameters)]

    return window_time, diameters, total_tension, centroid_diff, files_result
