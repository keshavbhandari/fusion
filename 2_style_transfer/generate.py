...
from utils.utils import flatten, unflatten, unflatten_corrupted, parse_generation
...
fusion_model = EncoderDecoderModel.from_pretrained(os.path.join(artifact_folder, "fusion", "style_transfer"))
fusion_model.eval()
...
output_dict = corruption_obj.apply_random_corruption(tokenized_sequence, context_before=5, context_after=1, meta_data=["jazz"], t_segment_ind=2)
corrupted_sequence = output_dict['corrupted_sequence']
corrupted_sequence = unflatten_corrupted(corrupted_sequence)
original_sequence = output_dict['original_sequence']
...
output_tokens = fusion_model.generate(input_ids=input_tokens.unsqueeze(0).to("cuda" if cuda_available() else "cpu"),
